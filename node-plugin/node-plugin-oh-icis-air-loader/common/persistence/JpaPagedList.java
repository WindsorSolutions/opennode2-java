...
public class JpaPagedList extends AbstractList {
   ...
   public JpaPagedList(Class entityClass, EntityManager em, String dataQuery, String countQuery, int cacheSize) {
   ...
   public JpaPagedList(Class entityClass, EntityManager em, int cacheSize) {
      this.entityClass = entityClass;
      this.em = em;
      this.cacheSize = cacheSize;
   }

   public final Serializable get(int index) {
      if(index > this.size()) {
         ...
         return (Serializable)this.cache.get(index - this.cacheStart);
      }
   }
   ...
   private Map getNamedParameters() {
