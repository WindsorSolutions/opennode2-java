...
import com.windsor.node.plugin.attains.dao.AttainsDao;
import com.windsor.node.plugin.attains.dao.AttainsDaoImpl;
...
/**
 * Provides an abstract service that the other ATTAINS services may extend.
 */
public abstract class AbstractAttainsService extends BaseWnosJaxbPlugin implements DisposableBean {
    ...
    private AttainsDao attainsDao;

    public AbstractAttainsService(OperationType operationType) {
        super();
    ...
    public void afterPropertiesSet() {
        ...
        EntityManagerFactory emf = provider.createEntityManagerFactory(getDataSources().get(ARG_DS_SOURCE),
                new PluginPersistenceConfig()
                        ...
                        .rootEntityPackage("com.windsor.node.plugin.attains.domain")
         ...
         * DAO setup
         */
        setAttainsDao(new AttainsDaoImpl(getEntityManager(), transactionDao));
        Assert.notNull(getAttainsDao(), "AttainsDao could not be located.");
    }
    ...
    protected AttainsDao getAttainsDao() {
        return attainsDao;
    }

    protected void setAttainsDao(AttainsDao attainsDao) {
        this.attainsDao = attainsDao;
    }
